...
class PgBench(object):
    ...
    def _init(self, scale):
        ...
        r = run_cmd(['psql', '-f', init_path, '-h', folders.SOCKET_PATH,
    ...
    def _run_custom(self, run, scale, duration, pgbench_init, read_only, nclients=1, njobs=1, aggregate=True, custom_query_folder=None):
        ...
        for query_file in custom_query_files:
            ...
            if len(dot_cnt) > 1 and match:
                ...
                # Add the custom query file path
                args.extend(['-f', query_file])

        # do an explicit checkpoint before each run
        run_cmd(['psql', '-h', folders.SOCKET_PATH,
                self._dbname, '-c', 'checkpoint'], env=self._env)
        log("pgbench: clients=%d, jobs=%d, aggregate=%s, read-only=%s, duration=%d" %
            (nclients, njobs, aggregate, read_only, duration))
    ...
    def run_custom_tests_pgbench(self):
        ...
        for i in range(0, self._iterations):
            ...
            for clients in self._clients:
                ...
                r = self._run_custom(i, scale, self._duration, self._pgbench_init,
                                     self._read_only, clients, clients, True, custom_queries_path)

...
def refresh_func1(conn, update_dir, stream, num_streams, verbose):
    ...
    try:
                ...
                for order_line in orders_gen:
                    ...
                    if todo_licols:
                        if todo_licols[0] != o_cols[0]:
                            log("bad data file for lineitem. Does not match orders key")
                            return 1
                        else:
        ...
        log("refresh function #1 failed. %s" % e)
        return 1


def refresh_func2(conn, delete_dir, stream, num_streams, verbose):
    ...
    try:
        if verbose:
            log("Running refresh function #2 in stream #%s" % stream)
        file_nr = stream + 1
        ...
        log("refresh function #2 failed. %s" % e)
        return 1
...
def run_Explain_query_stream_withCostOff(conn, query_root, generated_query_dir, stream, num_streams, result, verbose):
    ...
    for i in range(0, 22):
        try:
            ...
            if(order[i]==15 ):
               continue
            explain_query = "EXPLAIN (ANALYZE, BUFFERS,COSTS OFF ,FORMAT JSON) " + query  # Construct EXPLAIN command
...
def run_explain_query_withCostOff(query_root, update_dir, delete_dir, generated_query_dir, results_dir,
    ...
    """
    try:
        log("Explain test started with costs off ...")
        conn = pgdb.PGDB(host, database)
        result = r.Result("explain")
        stream = 3  # using static stream number for explain query
        ...
        if run_Explain_query_stream_withCostOff(conn, query_root, generated_query_dir, stream, num_streams, result, verbose):
            return 1
        #
        log("explain(with cost off) query finished.")
        if verbose:
            result.printMetrics()
...
def run_power_test(query_root, update_dir, delete_dir, generated_query_dir, results_dir,
    ...
    try:
        ...
        if not read_only:
            if refresh_func1(conn, update_dir, stream, num_streams, verbose):
                return 1
        ...
        result.startTimer()
        if not read_only:
            if refresh_func2(conn, delete_dir, stream, num_streams, verbose):
                return 1
        result.setMetric(REFRESH_METRIC % (stream, 2), result.stopTimer())
        log("Power tests finished.")
        if verbose:
            result.printMetrics()
...
def run_throughput_inner(query_root, data_dir, generated_query_dir,
                         host, database, stream, num_streams, queue, verbose):
    """
    ...
    try:
        ...
        if run_query_stream(conn, query_root, generated_query_dir, stream, num_streams, result, verbose):
            log("unable to finish query in stream #%s" % stream)
            exit(1)
        queue.put(result)
    except Exception as e:
        log("unable to connect to DB for query in stream #%s: %s" % (stream, e))
        exit(1)
...
def run_throughput_test(query_root, data_dir, update_dir, delete_dir, generated_query_dir, results_dir,
    ...
    try:
        ...
        for i in range(num_streams):
            stream = i + 1
            # queries
            log("Throughput tests in stream #%s started ..." % stream)
        ...
        for p in processes:
            p.join()
        log("Throughput tests finished.")
        for i in range(queue.qsize()):
            res = queue.get(False)
            if verbose:
                res.printMetrics()
            res.saveMetrics(results_dir, THROUGHPUT)
        #
...
def get_json_files(path):
    ...
    for mode in [POWER, THROUGHPUT]:
        sub_dir = os.path.join(path, mode)
        if os.path.exists(sub_dir) and os.path.isdir(sub_dir):
            json_files.extend(get_json_files_from(sub_dir))
    print(json_files)
    return json_files
...
def calc_metrics(results_dir, scale_factor, num_streams):

...
def build():
    ...
    with TemporaryFile() as strout:
        ...
        log("Configuring sources in '%s' with prefix '%s'" %
            (folders.REPOSITORY_PATH, folders.BUILD_PATH))
    ...
    with TemporaryFile() as strout:
        log("Building sources and installing into '%s'" %
            (folders.INSTALL_PATH,))

        # cleanup and build using multiple cpus
        build_start_time = datetime.now()
        b = subprocess.run(['make', '-s', 'clean'],
        ...
        subprocess.run(['make', '-s', '-j', str(cpu_count()), 'install'],
                       cwd=''.join(
                           [folders.BUILD_PATH, '/contrib/pg_stat_statements']),
                       capture_output=True, text=True)
    ...
    if (a.stderr != '' or b.stderr != '' or c.stderr != ''):
        log("Anomalies have been found while installing, please check logs in '%s'" % (
            folders.LOG_PATH,))
